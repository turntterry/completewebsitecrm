  ...
  updateDeploy: protectedProcedure
    .input(
      z.object({
        ...
        availabilityStartHour: z.number().int().min(5).max(12),
        availabilityEndHour: z.number().int().min(13).max(22),
        availabilityDaysAhead: z.number().int().min(1).max(21),
        availabilityPreferExternal: z.boolean(),
        slotPaddingMinutes: z.number().int().min(0).max(240),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await db
        ...
        .set({
          ...
          availabilityStartHour: input.availabilityStartHour,
          availabilityEndHour: input.availabilityEndHour,
          availabilityDaysAhead: input.availabilityDaysAhead,
          availabilityPreferExternal: input.availabilityPreferExternal,
          slotPaddingMinutes: input.slotPaddingMinutes,
