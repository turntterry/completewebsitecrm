...
import { instantQuotes, serviceConfigs, leads } from "../../drizzle/schema";
  ...
  submit: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const services = input.items.map((item) => ({
        ...
        serviceName: item.serviceType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
        sizeLabel: item.description ?? item.serviceType,
        sizeValue: (item.inputs?.sqft as number) ?? (item.inputs?.linearFeet as number) ?? 0,
  ...
  uploadPhoto: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        throw new Error("Photo upload is not configured. Please set AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and S3_BUCKET_NAME.");
      }
    }),
});
...
const galleryRouter = router({
  list: publicProcedure.query(async () => {
    ...
    return SEED_GALLERY.map((item, i) => ({ id: i + 1, ...item, visible: true }));
  }),
});
